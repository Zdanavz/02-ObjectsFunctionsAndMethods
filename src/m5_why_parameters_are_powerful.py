...
def main():
    ...
    #even_better_draw_circles(rg.Point(-200,0),30,8,'blue',3)
    even_better_draw_circles(rg.Point(0,0),30,3,'green',8)
    even_better_draw_circles(rg.Point(200,200),18,4,'Blue',3)
    even_better_draw_circles(rg.Point(-200,200),40,2,'Black',16)
...
def better_draw_circles(point, size):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        turtle.pen_down()
        turtle.draw_circle(size * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point, size, num, color, thick):
